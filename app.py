...
def recommend_places(top, longitude, latitude):
    cluster = kmeans_places.predict(np.array([longitude, latitude]).reshape(1, -1))[0]
    cluster_df = top[top['cluster'] == cluster].iloc[:5, [0, 1, 2, 3, 6, 7]]
    return cluster_df
...
def recommend_restaurants_endpoint():
    ...
    if longitude is not None and latitude is not None:
        recommended_places = recommend_places(top, longitude, latitude)
        recommended_malls = recommend_malls(top_malls, longitude, latitude)
        recommended_hotels = recommend_hotels(top_hotels, longitude, latitude)
        return jsonify({
